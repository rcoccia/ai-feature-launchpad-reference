...
explains the contract. Every adopter constitution includes six portable Control Tower baseline
...
| `FUN-AUTONOMY-01` | A later Mission or Constraints correction must be able to activate the installed strategy-change path without inventing or aliasing a constraint ID. |
...
Constraints asset and pin copied local method surfaces. `FUN-ARCHREVIEW-01` and `FUN-AUTONOMY-01`
are present in every constitution but become Change Record proof obligations only when architecture
or a Mission/Constraints strategy change is triggered, respectively.
...
`FUN-ARCHREVIEW-01` when load-bearing architecture is present and `FUN-AUTONOMY-01` when Mission or
Constraints strategy changes. Applicability and completeness remain Tower/human/reviewer judgments
except where a deterministic gate can prove a trigger with certainty.
