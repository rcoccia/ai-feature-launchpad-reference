...
This file is a **pointer only** (some agent harnesses auto-read `AGENTS.md`): the canonical file is
the single source of truth, so the method's activation is described in exactly one place and cannot
